...
async def main() -> None:
    ...
    async def message_cb(room: MatrixRoom, event: RoomMessageText) -> None:
        ...
        should_handle = True  # handle all messages we see to avoid silent drops
        LOGGER.info(
        ...
        try:
            ...
            reply = await call_sacred_brain(room.room_id, event, context)
        ...
        if reply:
            try:
                ...
                if BAIBOT_TTS_ENABLED:
                    asyncio.create_task(send_tts(reply, room.room_id, client))
            except Exception as exc:
...
async def call_sacred_brain(room_id: str, event: RoomMessageText, context: List[str]) -> str | None:
...
async def send_tts(text: str, room_id: str, client: AsyncClient) -> None:
    """
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=20.0) as http:
            ...
            resp = await http.post(BAIBOT_TTS_URL, headers=headers, json=payload)
            resp.raise_for_status()
        ...
        upload_resp = await client.upload(
            buf,
            content_type="audio/mpeg" if BAIBOT_TTS_FORMAT == "mp3" else f"audio/{BAIBOT_TTS_FORMAT}",
            filename=f"sam-tts.{BAIBOT_TTS_FORMAT}",
        )
        ...
        content = {
            ...
            "url": mxc_url,
            "info": {
                "mimetype": "audio/mpeg" if BAIBOT_TTS_FORMAT == "mp3" else f"audio/{BAIBOT_TTS_FORMAT}",
